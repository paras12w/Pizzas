...
// small — the floor guarantees entry, not size. Options-tagged trades
// (either a Discord alert flagged as an option, or a favorable-enough setup
// on a ticker with a real options chain — see ORGANIC_OPTION_BENEFIT_
// THRESHOLD in lib/scoring.js) size at half of whatever that would
// otherwise be: we track P&L against the *underlying's* price move, not the
// option's actual premium, and a given underlying move swings an option's
// value far more than the same dollar amount of shares would.
//
...
export const BOT_CONFIGS = {
  ...
  b: {
    id: "b",
    name: "Bot B",
    takeThreshold: 22, // enters looser setups Bot A would skip
    exitThreshold: 14,
