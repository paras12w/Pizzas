...
let memoryWatchlist = [];
let memoryPrevScores = { at: null, scores: {} };
...
// --- Previous cycle's confidence-per-ticker, for the trend arrow ---

export async function getPrevScores() {
  if (kvConfigured()) {
    const kv = await getKv();
    return (await kv.get(PREV_SCORES_KEY)) || { at: null, scores: {} };
  }
  return memoryPrevScores;
}

export async function savePrevScores(scores) {
  const entry = { at: Date.now(), scores };
  if (kvConfigured()) {
    const kv = await getKv();
