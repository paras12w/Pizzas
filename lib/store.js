...
export async function addAlert({ ticker, direction, instrument, note }) {
  const alert = {
    ticker: ticker.toUpperCase(),
    direction: direction || "bullish",
    instrument: instrument === "option" ? "option" : "stock",
    note: note || "",
    postedAt: Date.now(),
  };
...
// --- Full reset: clears test/live alert data and both bots back to a
// clean $10k start. Used by the "Reset" button on /alerts — deliberately
// leaves calibrated weights and the watchlist alone, since those aren't
// what testing pollutes. ---

export async function resetTradingState() {
  const freshA = freshBook();
  const freshB = freshBook();

  if (kvConfigured()) {
    const kv = await getKv();
    await Promise.all([
      kv.del(KEY),
      kv.del(ALERT_HISTORY_KEY),
      kv.del(TRACKED_KEY),
      kv.del(PREV_SCORES_KEY),
      kv.set(botKey("a"), freshA),
      kv.set(botKey("b"), freshB),
    ]);
  } else {
    memoryStore.clear();
    memoryAlertHistory = [];
    memoryTracked = {};
    memoryPrevScores = { at: null, scores: {}, ranks: { buy: {}, sell: {} }, tickers: { buy: [], sell: [] } };
    memoryBotBooks.set("a", freshA);
    memoryBotBooks.set("b", freshB);
  }
}
