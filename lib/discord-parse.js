...
// Options carry far more leverage/volatility per dollar than the underlying
// stock — checked independently of ticker extraction (a message can name a
// $TICKER cashtag *and* separately reference an option leg or vocabulary
// elsewhere), since this drives a position-sizing dampener downstream in
// lib/bot.js, not just the ticker match itself.
const OPTION_LEG = /\b[A-Za-z]{1,5}\s?\d+(\.\d+)?[CP]\b/i; // e.g. "450C", "QQQ 450C"
const OPTION_WORDS = /\b(calls?|puts?|options?|contracts?|strike|expir(?:y|ation)|premium)\b/i;
...
export function parseAlertText(raw) {
  ...
  const instrument = OPTION_LEG.test(text) || OPTION_WORDS.test(text) ? "option" : "stock";

  return { ticker, direction, instrument, note: text.slice(0, 280) };
}
