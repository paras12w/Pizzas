...
export async function fetchTickerSnapshot(ticker) {
  const snapshot = {
    ...
    // Next/most-recent earnings date for the "earnings catalyst" benefit
    // read in lib/scoring.js — free on the same quote() call. Yahoo notes
    // this can be off by up to ~2 days, which scoring.js's decay curve is
    // deliberately wide enough to absorb.
    earningsAt: null,
    valid: false,
  };
  ...
  try {
    ...
    snapshot.earningsAt = quote.earningsTimestamp ? quote.earningsTimestamp.getTime() : null;
