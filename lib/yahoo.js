...
/**
 * Live market-mover tickers (most active + today's top gainers) straight
 * from Yahoo. Used as a fallback candidate source when Reddit's public JSON
 * endpoints don't yield anything — which happens often from cloud/serverless
 * IP ranges (including Vercel's) regardless of User-Agent, since Reddit
 * blocks a lot of datacenter traffic outright. This keeps the board
 * populated with real, currently-active tickers even when Reddit 403s.
 */
export async function fetchMarketMovers(count = 20) {
  try {
