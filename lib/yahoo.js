...
export async function fetchMarketMovers(count = 15) {
  try {
    ...
    // Losers listed before gainers: Reddit-sourced candidates already skew
    // bullish, so when the merged pool gets capped downstream, losers are
    // the ones actually filling a gap and should win the limited slots.
    const symbols = [
