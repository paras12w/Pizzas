...
// Rebalanced to make room for `structure` (multi-day trend/swing read) and
// `news` (fresh-headline catalyst read) — every other weight below was
// scaled down proportionally from its old value rather than picked fresh,
// so the *relative* importance of the original signals to each other is
// unchanged, just their share of the total now that there are two more
// components splitting it.
export const DEFAULT_WEIGHTS = {
  confidence: {
    redditStrength: 0.144,
    cashtagStrength: 0.104,
    mentionBreadth: 0.048,
    momentum: 0.136,
    volume: 0.12,
    rangePosition: 0.04,
    sentiment: 0.072,
...
export function scoreTicker(redditEntry, yahooSnap, discordAlert, maxWeightedScore, weights, newsEntry) {
  ...
  // Organic options selection. A Discord alert's explicit instrument choice
  // always wins over this. Otherwise, two paths to trading as an option
  // instead of plain shares:
  ...
  return {
    ...
    instrument: discordAlert?.instrument || (organicOption ? "option" : "stock"),
    sources: {
      reddit: !!redditEntry,
