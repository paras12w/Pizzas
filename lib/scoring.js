...
export function scoreTicker(redditEntry, yahooSnap, discordAlert, maxWeightedScore, weights) {
  ...
  // Which signal sources actually exist for this ticker, vs. just scoring
  // zero because there's genuinely nothing there. This matters because a
  // large fraction of candidates (anything sourced from Yahoo's own movers
  // rather than a Reddit mention — the common case whenever Reddit is
  // unreachable, which cloud IPs hit often) never had a Reddit entry to
  // begin with. Scoring that absence the same as a real zero silently
  // capped confidence at whatever the non-Reddit weights sum to (~45 with
  // the defaults below) — below even Bot B's 60 threshold — so a ticker
  // with no Reddit footprint could *never* be taken organically, no matter
  // how strong its price/volume signal was. Renormalizing below against
  // only the weight of sources that actually exist for this ticker fixes
  // that without changing how any *present* signal is weighted relative to
  // the others.
  const hasReddit = !!redditEntry;
  const hasAnalyst = yahooSnap?.analystRating != null;
  const hasDiscordAlert = !!discordAlert;
  ...
  const confidenceWeightAvailable =
    (hasReddit ? w.confidence.redditStrength + w.confidence.cashtagStrength + w.confidence.mentionBreadth + w.confidence.sentiment : 0) +
    w.confidence.momentum + w.confidence.volume + w.confidence.rangePosition +
    (hasAnalyst ? w.confidence.analystRating : 0) +
    (hasDiscordAlert ? w.confidence.discord : 0);
  let confidence = confidenceWeightAvailable > 0
    ? clamp(confidenceParts.reduce((sum, p) => sum + p.pts, 0) / confidenceWeightAvailable)
    : 0;
  ...
  const benefitWeightAvailable =
    w.benefit.momentum + w.benefit.volume + w.benefit.rangePosition +
    (yahooSnap?.impliedVolatility != null ? w.benefit.iv : 0) +
    (yahooSnap?.putCallRatio != null ? w.benefit.skew : 0) +
    (hasAnalyst ? w.benefit.analystRating : 0) +
    (hasDiscordAlert ? w.benefit.discord : 0);
  const benefit = benefitWeightAvailable > 0
    ? clamp(benefitParts.reduce((sum, p) => sum + p.pts, 0) / benefitWeightAvailable)
    : 0;
