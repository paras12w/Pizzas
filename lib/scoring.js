...
// `news` (fresh-headline catalyst read), `swingTiming` (RSI-based entry
// timing), and `earningsCatalyst` (proximity to an earnings date,
// benefit-only — see below) — every other weight below was scaled down
...
export const DEFAULT_WEIGHTS = {
  ...
  benefit: {
    ...
    iv: 0.1175,
    skew: 0.0619,
    analystRating: 0.0548,
    discord: 0.0486,
    structure: 0.1325,
    news: 0.0619,
    swingTiming: 0.0564,
    earningsCatalyst: 0.06,
  },
};
...
export function scoreTicker(redditEntry, yahooSnap, discordAlert, maxWeightedScore, weights, newsEntry) {
  ...
  // "Earnings catalyst" — benefit-only (not confidence: a nearby earnings
  // date says nothing about which direction a move goes, only that a
  // bigger one is plausible). Peaks right at the event and tapers off over
  // roughly two weeks either side, wide enough to absorb Yahoo's own
  // documented ~2-day inaccuracy on this field. Not gated to "upcoming
  // only" — a report from a day or two ago is still live catalyst
  // territory (post-earnings drift), and momentum/structure above already
  // capture whatever's already priced in.
  const daysToEarnings = yahooSnap?.earningsAt != null ? (yahooSnap.earningsAt - Date.now()) / 86400000 : null;
  const earningsCatalystStrength = daysToEarnings != null ? clamp(100 - Math.abs(daysToEarnings) * 7) : 0;
  ...
  const benefitParts = [
    ...
    { key: "earningsCatalyst", label: "Earnings catalyst", pts: earningsCatalystStrength * w.benefit.earningsCatalyst, detail: daysToEarnings != null ? (daysToEarnings >= 0 ? `earnings in ~${Math.round(daysToEarnings)}d` : `earnings ~${Math.round(-daysToEarnings)}d ago`) : "no earnings date" },
